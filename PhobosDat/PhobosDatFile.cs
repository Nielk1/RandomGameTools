...
namespace PhobosDat
{
    class PhobosDatFile
    {
        ...
        public PhobosDatFile(Stream data)
        {
            ...
            if (buffer[0] != (byte)'R' || buffer[1] != (byte)'C' || buffer[2] != (byte)'F' || buffer[3] != (byte)'S')
            {
                throw new FormatException("Type indicator RFCS not found");
            }
